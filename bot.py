...
BOT_PREFIX = ('/')
TOKEN = 'NzM5OTQwMjAxNDQyNTc0NDU2.XyhxAw.yxFh6tnJSH8yVyQ4QikS8CXhdsA'
OWNERS = [376731930982613002]
BLACKLIST = []
client = Bot(command_prefix=BOT_PREFIX)

async def status_task():
	while True:
		await client.change_presence(activity=discord.Game("Sur le serveur"))
		await asyncio.sleep(10)
		await client.change_presence(activity=discord.Game("Bonne Journée!!"))
		await asyncio.sleep(10)
		await client.change_presence(activity=discord.Game("/ help"))
		await asyncio.sleep(10)
		await client.change_presence(activity=discord.Game("Ici pour vous aidez!!!"))
		await asyncio.sleep(10)
...
async def close(context):
	if context.message.author.id in BLACKLIST:
		...
		embed = discord.Embed(title=':information_source: (COMMANDE EN DEV) Info',
							  description='Le Channel va etre fermé',
							  color=0x00FF00)
		await context.message.channel.send(embed=embed)
...
async def bitcoin(context):
	...
	async with aiohttp.ClientSession() as session:  # Async HTTP request
		...
		embed = discord.Embed(title=':information_source: Info',
							  description='Le prix d\'un Bitcoin est: $' + response['bpi']['USD']['rate'], color=0x00FF00)
		await context.message.channel.send(embed=embed)
...
async def shutdown(context):
	if context.message.author.id in BLACKLIST:
		...
		if context.message.author.id in OWNERS:
			embed = discord.Embed(title='Shutdown!', description='Arret du bot. Ciao!!! :wave:', color=0x00FF00)
			await context.message.channel.send(embed=embed)
			await client.logout()
			await client.close()
		else:
	...
	if context.message.author.id in BLACKLIST:
		...
		if context.message.author.guild_permissions.administrator:
			...
			embed = discord.Embed(title='Warn!',
								  description='**{0}** a pris un warn de **{1}**!'.format(member, context.message.author),
								  color=0x00FF00)
			embed.add_field(name='Raison:', value=mesg)
			await context.message.channel.send(embed=embed)
			await context.message.delete()
			await member.send('Vous avait été warn par **{0}**!  '.format(
				context.message.author) + 'Raison: {0}'.format(mesg))
		else:
			embed = discord.Embed(title='Erreur!', description='You don\'t have the permission to use this command.',
								  color=0x00FF00)
			await context.message.channel.send(embed=embed)
...
async def purge(context, number):
	if context.message.author.id in BLACKLIST:
		...
		if context.message.author.guild_permissions.administrator:
			...
			embed = discord.Embed(title='Chat Clear!',
								  description='**{0}** A Supprimé **{1}** messages!'.format(context.message.author,
...
async def help(context):
	if context.message.author.id in BLACKLIST:
		...
		embed.add_field(name='Invite - Invite le bot', value='Usage: / invite', inline=False)
		embed.add_field(name='Server - Rejoindre le server', value='Usage: / server', inline=False)
		...
		embed.add_field(name='Bitcoin - Regarder le courant du bitcoin xDD', value='Usage: / bitcoin', inline=False)
		embed.add_field(name='Info - recevoir les info du bot', value='Usage: / info', inline=False)
		embed.add_field(name='Shutdown - Arreter le bot [OWNER]', value='Usage: / shutdown', inline=False)
		embed.add_field(name='Say - Envoyez le message de votre choix [OWNER]', value='Usage: / say <message>', inline=False)
		embed.add_field(name='Embed - I send a embed message of your choice [OWNER]', value='Usage: / embed <message>', inline=False)
		embed.add_field(name='Kick - Kicker un utilisateur', value='Usage: / kick <user> <reason>', inline=False)
		embed.add_field(name='Ban - bannir un utilisateur', value='Usage: / ban <user> <reason>', inline=False)
		embed.add_field(name='Warn - Warn un utilisateur en mp', value='Usage: / warn <user> <reason>', inline=False)
		embed.add_field(name='Unban - debannir un utilisateur', value='Usage: / unban <user>', inline=False)
		embed.add_field(name='Purge - Supprimer un nombre de message donné', value='Usage: / purge <number>', inline=False)
		embed.add_field(name='Help - Accedez a ce menu', value='Usage: / help', inline=False)
		await context.message.channel.send(embed=embed)
...
client.run("NzM5OTQwMjAxNDQyNTc0NDU2.XyhxAw.yxFh6tnJSH8yVyQ4QikS8CXhdsA")